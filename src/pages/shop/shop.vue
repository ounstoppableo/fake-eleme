 <template>
  <div>
    <section v-if="!showLoading" class="shop_container" ref="head">
      ...
      >
        ...
        <section class="description_header">
          <router-link
            :to="{ path: 'shop/shopDetail', query: { shopDetailData } }"
          ...
          >
            ...
            <section class="description_right">
              ...
              <p class="description_text">
                商家配送／{{ ratingScoresData.deliver_time }}分钟送达／配送费¥{{
      ...
      <transition name="fade-choose">
        <section v-show="changeShowType == 'food'" class="food_container">
                      ...
                      <section class="menu_food_description">
                        <h3 class="food_description_head">
                          ...
                          >
                            <li
                              v-if="attribute"
                              v-for="(attribute, attrindex) in foods.attributes"
                              :key="attrindex"
                              :style="{
          ...
          <section class="buy_cart_container" ref="footer">
      ...
      <transition name="fade-choose">
        ...
        >
          <section v-load-more="loaderMoreRating">
            <section>
...
export default {
  data() {
    return {
      shopId: this.$route.query.id, //商铺ID
      ...
      promotionInfo: "", //公告
      showActivities: false, //商家支持的活动
      changeShowType: "food", //选择展示的页面
      menuList: [], //菜单标签页
      menuIndex: 0, //菜单活页
      categoryNum: [], //不同分类的商品数
      TitleDetailIndex: -1, //查看商品标题细节的选项
      showSpecs: false, //查看规格
      choosedFoods: {}, //选中的食物
      allFoodList: [], //所有食物列表
      cartFoodList: [], //购物车列表
      specsIndex: 0, //规格索引
      timerForTip: null, //tip的计时器
      totalPrice: 0, //总价格
      totalNum: 0, //总数量
      deliveryFee: 0, //运输费用
      showDeleteTip: false, //规格商品删除提示
      showCartList: false, //查看购物车列表
      minimumOrderAmount: 0, //最低配送费
      ratingScoresData: [], //评论分数信息
      ratingTagsList: [], //评论分类信息
      ratingTageIndex: 0, //批量分类索引
      ratingList: [], //评论列表
      reqIndex: 0, //评论请求索引
      loadRatings: false, //加载评论
      loaderMoreRating: false, //加载更多评论
      receiveInCart: false, //购物车缩放动画
      timerForAddFood: null, //添加食物的定时器
    };
  },
  computed: {
    ...mapState(["geohash"]),
  ...
  watch: {
    //监听所有食物表，变化了就更新展示的商品列表
    allFoodList() {
      this.cartFoodList = this.allFoodList.filter(
        (item) => item.restaurant_id === this.shopId
      );
    },
    //监听购物车列表，为了计算总价格
    cartFoodList() {
      this.totalPrice = this.cartFoodList
        .reduce((pre, item) => pre + item.price * item.num, 0)
        .toFixed(2);
      this.totalNum = this.cartFoodList.reduce(
        (pre, item) => pre + item.num,
        0
      );
    },
    //监听总价格变化，为了计算是否满足最低配送费要求
    totalPrice(newValue) {
      this.minimumOrderAmount =
        this.shopDetailData.float_minimum_order_amount - newValue;
    },
    //评论区加载更多模块
    loaderMoreRating(newValue) {
      if (newValue === true) {
        this.loadRatings = true;
        this.reqIndex++;
        getRatings(this.shopId, this.reqIndex).then((req) => {
          req.data.forEach((item) => this.ratingList.push(item));
          this.loadRatings = false;
          this.loaderMoreRating = false;
        });
      }
    },
    //购物车添加物品时缩放动画
    receiveInCart(newValue) {
      if (newValue) {
        if (!this.timerForAddFood) {
          this.timerForAddFood = setTimeout(() => {
            this.receiveInCart = false;
            this.timerForAddFood = null;
          }, 500);
        }
      }
    },
  ...
  methods: {
    ...
    chooseMenu(e, index) {
      ...
      let i = 0;
      for (; i < this.$refs.menuFoodList.children[0].children.length; i++) {
        if (
          +this.$refs.menuFoodList.children[0].children[i].dataset.id === index
        ) {
          break;
        }
      }
      this.$refs.menuFoodList.scrollTo({
        top: this.$refs.menuFoodList.children[0].children[i].offsetTop,
        behavior: "smooth",
      });
    },
    //查看商品标题细节
    showTitleDetail(index) {
      this.TitleDetailIndex = this.TitleDetailIndex >= 0 ? -1 : index;
    },
    //展开购物车清单
    toggleCartList() {
    ...
    //监听底部购物车商品的变化
    listenCart(value) {
      const index = this.allFoodList.findIndex(
        (item) => item.foodId === value.foodId && item.specs === value.specs
      );
      if (index !== -1) {
        //如果数量为0，则从购物车删除
        if (value.num === 0) {
          this.allFoodList.splice(index, 1);
        } else {
          this.receiveInCart = true;
          this.$set(this.allFoodList, index, value);
        }
      } else {
        this.receiveInCart = true;
        this.allFoodList.push(value);
      }
      window.localStorage.setItem("buycart", JSON.stringify(this.allFoodList));
    },
    //从购物车移除
    removeOutCart(value) {
      value.num--;
      this.listenCart(value);
      const index = this.$refs[`buyCart-${this.shopId}`].findIndex(
        (item) => item.foods.specfoods[0].item_id === value.itemId
      );
      this.$refs[`buyCart-${this.shopId}`][index].foodNum--;
    },
    //购物车添加
    addToCart(value) {
      value.num++;
    ...
    //查看选规格详情
    showChooseList(value) {
      this.showSpecs = !this.showSpecs;
      this.choosedFoods = value;
    },
    //选规格
    chooseSpecs(index) {
    ...
    clearCart() {
      ...
      this.allFoodList = this.allFoodList.filter(
        (item) => item.restaurant_id !== this.shopId
      );
      this.$refs[`buyCart-${this.shopId}`].forEach((item) => {
        item.foodNum = 0;
      });
      window.localStorage.setItem("buycart", JSON.stringify(this.allFoodList));
    },
    ...
    addSpecs(
      ...
      const item = this.cartFoodList.filter(
        (item) => item.foodId === food_id && item.specs === specs
      );
      const num = item.length ? item[0].num + 1 : 1;
      const value = {
    ...
    showMoveDotFun(e) {
      ...
      //获取终点坐标
      const final = this.$refs.cartContainer.getBoundingClientRect();
      document.documentElement.style.setProperty(
        "--finalX",
        final.x - start.x + (final.width * 1) / 4 + "px"
      );
      document.documentElement.style.setProperty(
        "--finalY",
        final.y - start.y + (1 / 2) * final.height + "px"
      );
  ...
  directives: {
    loadMore: {
      inserted: (ele, _, vnode) => {
        ele.parentNode.addEventListener("scroll", () => {
          if (
            Math.ceil(ele.parentNode.scrollTop + ele.parentNode.clientHeight) >=
            ele.parentNode.scrollHeight
          ) {
            vnode.context.loaderMoreRating = true;
          }
        });
      },
    },
  ...
  //初始化数据
  async beforeMount() {
    const getShopDetailRes = await getShopDetail(this.shopId);
    this.shopDetailData = getShopDetailRes.data;
    this.deliveryFee = this.shopDetailData.float_delivery_fee;
    this.minimumOrderAmount = this.shopDetailData.float_minimum_order_amount;
    this.promotionInfo = this.shopDetailData.promotion_info;

    const getFoodsRes = await getFoods(this.shopId);
    this.menuList = getFoodsRes.data;
    this.menuList.forEach((item) => this.categoryNum.push(item.foods.length));

    const getScoresRes = await getScores(this.shopId);
    this.ratingScoresData = getScoresRes.data;

    const getRatingTagsRes = await getRatingTags(this.shopId);
    this.ratingTagsList = getRatingTagsRes.data;

    const getRatingsRes = await getRatings(this.shopId, this.reqIndex);
    this.ratingList = getRatingsRes.data;

    if (this.menuList.length) {
      this.showLoading = false;
    }
    this.allFoodList = JSON.parse(window.localStorage.getItem("buycart"))
      ? JSON.parse(window.localStorage.getItem("buycart"))
      : [];
  },
};
</script>

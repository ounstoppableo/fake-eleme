...
import { searchShop } from "@/service/getData";
...
export default {
  ...
  methods: {
    searchTarget(shopName) {
      //修改历史记录的显示
      this.showHistory = false;
      //如果传入了参数(点击搜索记录)
      if (shopName) {
        (async () => {
          const shops = await searchShop(this.geohash, shopName);
          this.restaurantList = shops.data;
        ...
        (async () => {
          if (!this.searchValue) return alert("输入不能为空");
          //添加历史记录到本地存储
          ...
          const shops = await searchShop(this.geohash, this.searchValue);
          this.restaurantList = shops.data;
          //修改emptyResult的显示
          if (!this.restaurantList.length) {
            this.emptyResult = true;
